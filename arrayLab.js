...
// 1. Filter the array of inventors into a new array containing only the inventors born in the 1500's
  ...
  const bornBefore1500 = inventors.filter((person) => {
...
// 2. Map the array of the inventors into a new array containing objects with just the first and last names as properties
  ...
  const mappedInventors = inventors.map(person => {
    const obj = {};
...
// 3. Sort the inventors by birthdate, oldest to youngest
  inventors.sort((firstItem, secondItem) => {
...
inventors.sort((firstItem, secondItem) => {
  return (firstItem.passed - firstItem.year) - (secondItem.passed - secondItem.year);
});
console.log(inventors);
...
let yearsLivedArr = inventors.reduce((acc, inventor) => {
  ...
  obj.yearsLived = inventor.passed - inventor.year;
  acc.push(obj);
  return acc;
}, [])

const yearsLivedNum = inventors.reduce((acc, inventor) => {
  return acc + (inventor.passed - inventor.year);
}, 0);
console.log(yearsLivedArr);
console.log(yearsLivedNum);
